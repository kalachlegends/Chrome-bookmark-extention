...
function rightClick(e) {
    e.preventDefault();
    ContextMenuGlobal.allElementVal = [...e.path[1].options]
    ContextMenuGlobal.id = e.path[0].value
    ContextMenuGlobal.link = e.path[0].dataset.href
    ContextMenuGlobal.element = e.path[0]
    ContextMenuGlobal.selectedLorR = e.path[1].id
    ContextMenuGlobal.folder = parseInt(e.path[0].dataset.folder)
    ContextMenuGlobal.selected = e.path[1].id
    ContextMenuGlobal.text = replaceFolderString(e.path[0].text)
    if (true == ContextMenuGlobal.folder) ContextMenuGlobal.text = replaceFolderString(e.path[0].text)
    else ContextMenuGlobal.text = e.path[0].text
    ...
    let optionSelected = ContextMenuGlobal.allElementVal.filter(option => option.selected)
    ...
    optionSelected = ContextMenuGlobal.allElementVal.filter(option => option.selected)
    if (optionSelected.length == 0) {
        console.log("---as")
        addSelectOptionByValue(e.path[1].id, ContextMenuGlobal.id)
    }
    if (e.path[1].id == "rightSelect") {
        moveToLeftSelectBtn.show()
        moveToRightSelectBtn.hide()

    }
    if (e.path[1].id == "leftSelect") {
...
$(".trash").click(function () {
    try {
        let filterValue = ContextMenuGlobal.allElementVal.filter(option => option.selected).map(option => option.value);
        if (filterValue == 0) throw new Error("No selected items!")
        let allFolder = ContextMenuGlobal.allElementVal.filter(option => option.selected).map(option => option.dataset.folder);
        let selectedElement = ContextMenuGlobal.allElementVal.filter(option => option.selected)
...
$(".moveUp").click(function () {
    try {
        ...
        let element, filterValue, selectedElement, select = this.dataset.select, currentId
...
function addSetAtributHtmlArray(arrayLeft, idArray) {
    setTimeout(function () {
        idArray.forEach(function (e) {
            console.log(document.querySelector("#" + arrayLeft + ` option[value="${e}"]`))
            document.querySelector("#" + arrayLeft + ` option[value="${e}"]`).setAttribute('selected', true);
        })
    }, 55)
...
function addSelectOptionByValue(selected, id) {
    ...
    document.querySelector("#" + selected + ` option[value="${id}"]`).setAttribute('selected', true);
}
...
function moveUpOrFall(boolean, array) {
    if (boolean == true) {
        for (let i = 0; i < array.length; i++) {
            const e = array[i];
            let id = e.value, index = e.dataset.index, parentId = e.dataset.parent
        ...
        for (i; i >= 0; i--) {
            const e = array[i];
            let id = e.value, index = parseInt(e.dataset.index), parentId = e.dataset.parent
...
function parserIni(text) {
    ...
    let mathTitle = text.match(/\[0\]\$\s+[^&]+(\s>+(\[\d+\])?(#|\$)\s+[^&]+(&|%)(\s+[^&|%]+?)?\n)+/g), currentId = getCurrentID("leftSelect"),
        resultFolder = [], resultUrl = []
    console.log(mathTitle)
    if (mathTitleAndLink != null) {
        mathTitleAndLink.reverse().forEach(element => {
            let title = element.match(/#\s+[^&]+/)[0].replace(/#\s+/, ""), url = element.match(/&\s+.+/g)[0].replace(/& /, "")
            createBookmark(0, url, title, currentId,
                function (e) {
    ...
    if (mathTitle != null) {
        mathTitle.forEach(element => {
            let arrayArrow = element.match(/\[\d+\]/g), lengthArray = parseInt(arrayArrow[arrayArrow.length - 1].replace(/(\[|\])/g, ""))
...
function insertForTxt(element, iteration, lastIteration, idParent, resultFolder = [], booleanParent = true) {
    let stringArrow = "", folderRegex, regexLinks, mathLiks, mathFoldersAndID, matchFolfders
    ...
    if (booleanParent) {
        createBookmark(0, "", titleFolder.replace(/\[0\]\$/g, "").trim(), idParent,
            function (e) {
                console.log(e)
                resultFolder.push({ i: iteration, folder: e, stringArrow: stringArrow })
            })
        ...
        if (titleFolder != undefined) {
            titleFolder.forEach(((e) => {
                let iteration = parseInt(e.match(/\[\d+\]/g)[0].replace(/(\[|\])/g, "")), stringArrows = e.match(/>+/g)[0]
                matchFolfders = findByIterationAndArrow(resultFolder, stringArrows), titile = e.replace(/\[0\]\$/g, "")

                createBookmark(0, "", titile, matchFolfders.folder.id,
                    function (e) {

                        resultFolder.push({ i: iteration, folder: e, stringArrow: stringArrows + ">" })
                    })
            }))
        }
    ...
    setTimeout(function () {
        ...
        else insertForTxt(element, iteration + 1, lastIteration, idParent, resultFolder, false)
    }, 50)
...
function findByIterationAndArrow(array, iteration, stringArrow) {
    return array.filter(element => element.i == iteration && element.stringArrow == stringArrow)[0];
}